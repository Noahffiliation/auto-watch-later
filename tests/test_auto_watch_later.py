...
def test_get_subscriptions(mock_youtube_client):
    ...
    response2 = {"items": [{"snippet": {"resourceId": {"channelId": "c2"}}}]}
    ...
    mock_youtube_client.subscriptions().list_next.side_effect = [
        mock_list_request,
        None,
    ]
...
def test_get_channel_shorts_video_ids(mock_youtube_client):
    ...
    shorts = auto_watch_later.get_channel_shorts_video_ids(
        mock_youtube_client, "UC123", "2024-01-01T00:00:00Z"
    )
...
def test_add_to_watch_later(mock_youtube_client, mocker):
    ...
    added_count, remaining = auto_watch_later.add_to_watch_later(
        mock_youtube_client, [{"id": "v1"}, {"id": "v2"}], "PL123"
    )
...
def test_get_new_videos_with_shorts_filtering(mock_youtube_client, mocker):
    ...
    mocker.patch(
        ...
        return_value=[{"id": "v1", "title": "New Video", "channel": "C1"}],
    )

    videos, scan_state = auto_watch_later.get_new_videos_with_shorts_filtering(
        mock_youtube_client, ["c1"], "2025-01-01Z"
    )
...
def test_process_channel_batch(mock_youtube_client, mocker):
    ...
    videos = auto_watch_later.process_channel_batch(
        mock_youtube_client, ["c1"], "2025-01-01Z", set()
    )
    assert len(videos) == 1
...
def test_get_client_credentials_file_installed(mocker):
    ...
    mocker.patch("os.path.exists", return_value=True)
    mock_open_func = mock_open(
        read_data='{"installed": {"client_id": "file_id", "client_secret": "file_secret"}}'
    )
    mocker.patch("builtins.open", mock_open_func)
...
def test_get_client_credentials_file_web(mocker):
    ...
    mocker.patch("os.path.exists", return_value=True)
    mock_open_func = mock_open(
        read_data='{"web": {"client_id": "file_id_web", "client_secret": "file_secret_web"}}'
    )
    mocker.patch("builtins.open", mock_open_func)
...
def test_env_bool(mocker):
    mocker.patch.dict(
        "os.environ",
        {"TEST_TRUE": "true", "TEST_FALSE": "false", "TEST_OTHER": "maybe"},
    )
...
def test_subscriptions_cache_loading_and_saving(mocker):
    mocker.patch(
        "os.path.exists",
        side_effect=lambda path: path == auto_watch_later.SUBSCRIPTIONS_CACHE_FILE,
    )
...
def test_scan_progress_io(mocker):
    ...
    mocker.patch(
        "builtins.open",
        mock_open(read_data='{"last_channel_index": 2, "shorts_cache": ["s1"]}'),
    )
    assert auto_watch_later.load_scan_progress() == {
        ...
        "items": [
            {
                ...
                "contentDetails": {"videoId": "old_vid"},
            }
        ]
    }
    cutoff = datetime.datetime(2025, 1, 1, tzinfo=datetime.timezone.utc)
...
def test_get_channel_shorts_video_ids_cases(mock_youtube_client, mocker):
    ...
    res = auto_watch_later.get_channel_shorts_video_ids(
        mock_youtube_client, "invalid", "2025-01-01Z"
    )
    assert res == set()
...
def test_build_shorts_cache_for_channels_cases(mock_youtube_client, mocker):
    ...
    mocker.patch(
        "auto_watch_later.get_channel_shorts_video_ids",
        side_effect=[Exception("Error UC1"), {"s2"}],
    )
    res = auto_watch_later.build_shorts_cache_for_channels(
        mock_youtube_client, ["UC1", "UC2"], "2025-01-01Z"
    )
    assert res == {"s2"}
    ...
    with pytest.raises(auto_watch_later.QuotaExceededException):
        auto_watch_later.build_shorts_cache_for_channels(
            mock_youtube_client, ["UC1"], "2025-01-01Z"
        )
...
def test_filter_videos_short_playlist(mocker):
    mocker.patch("auto_watch_later.SHORT_PLAYLIST", True)
    mocker.patch("auto_watch_later.INCLUDE_SHORTS", False)
...
def test_get_videos_from_activities_exception(mock_youtube_client):
    ...
    res = auto_watch_later.get_videos_from_activities(
        mock_youtube_client, "UC123", "2025-01-01Z", set()
    )
    assert res is None
...
def test_get_videos_from_search_cases(mock_youtube_client):
    ...
    auto_watch_later.get_videos_from_search(
        mock_youtube_client, "UC123", "2025-01-01T00:00:00+00:00", set()
    )
    ...
    res = auto_watch_later.get_videos_from_search(
        mock_youtube_client, "UC123", "2025-01-01Z", set()
    )
    assert res == []
...
def test_get_new_videos_with_shorts_filtering_resume(mock_youtube_client, mocker):
    ...
    resume = {"last_channel_index": 1, "shorts_cache": ["s1"]}
...
def test_main_resume_and_exception_paths(mocker):
    ...
    mocker.patch(
        "auto_watch_later.get_new_videos_with_shorts_filtering",
        return_value=([], {"last_channel_index": 1, "shorts_cache": set()}),
    )
    mocker.patch("auto_watch_later.save_check_time")
...
def test_fetch_playlist_page_edges(mock_youtube_client, mocker):
    ...
    mock_req_max.execute.return_value = {
        "items": [
            {
                "snippet": {"publishedAt": new_time},
                "contentDetails": {"videoId": "vid_max"},
            }
        ]
    }
    ...
    assert (
        auto_watch_later.fetch_playlist_page(
            mock_youtube_client, mock_req_max, cutoff, shorts_filled, 1
        )
        is None
    )
...
def test_load_playlist_id_cache_invalid_json(mocker):
    mocker.patch("os.path.exists", return_value=True)
    mocker.patch("builtins.open", mock_open(read_data="{invalid-json"))
    cache = auto_watch_later._load_playlist_id_cache()
    assert cache == {}


def test_add_videos_to_playlists_with_shorts_playlist(mock_youtube_client, mocker):
    videos = [
...
def test_main_short_playlist_enabled(mocker, monkeypatch):
    monkeypatch.setattr(auto_watch_later, "SHORT_PLAYLIST", True)
    mocker.patch("auto_watch_later.setup_logging")
    mocker.patch("auto_watch_later.cleanup_logging")
    mocker.patch("auto_watch_later.get_authenticated_service")
    mocker.patch("auto_watch_later.check_quota_usage", return_value=True)
    mock_get_pl = mocker.patch(
